...
export function generateBadges(repo: EnhancedRepo): string {
  ...
  if (frameworks.includes("Python")) {
       badges.push(`<img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white" alt="Python"/>`);
  }
  if (frameworks.includes("Go")) {
      badges.push(`<img src="https://img.shields.io/badge/Go-00ADD8?logo=go&logoColor=white" alt="Go"/>`);
  }
  if (type === "frontend" || frameworks.includes("React")) {
      badges.push(`<img src="https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black" alt="React"/>`);
  }
  if (frameworks.includes("Next.js")) {
      badges.push(`<img src="https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white" alt="Next.js"/>`);
  }
  ...
  if (repo.language === "TypeScript") {
      badges.push(`<img src="https://img.shields.io/badge/TypeScript-3178C6?logo=typescript&logoColor=white" alt="TS"/>`);
  }
  
  // Cocov Badge
  if (repo.stack?.dependencies?.includes("cocov") || repo.stack?.devDependencies?.includes("cocov")) {
      badges.push(`<a href="https://cocov.vercel.app/${repo.owner.login}/${repo.name}"><img src="https://cocov.vercel.app/api/badge/${repo.owner.login}/${repo.name}" alt="Cocov"/></a>`);
  }
