 ...
 */
export async function detectStackFromRemote(repo: GithubRepo, allRepoNames: string[] = []): Promise<RepoStack> {
  ...
  if (pkgJson) {
    try {
      ...
      if (allRepoNames.length > 0) {
        stack.internalDependencies = allRepoNames.filter(r => depKeys.includes(r) || depKeys.some(d => d.endsWith(`/${r}`)));
      }

      /* v8 ignore start */
      if (depKeys.includes("react") || depKeys.includes("next") || depKeys.includes("vue")) {
        stack.type = depKeys.includes("express") || depKeys.includes("nest") ? "fullstack" : "frontend";
        ...
        if (stack.type === "fullstack") {
             ...
             if (depKeys.includes("nestjs") || depKeys.some(d => d.includes("@nestjs"))) stack.frameworks.push("NestJS");
        }
      } else if (depKeys.includes("express") || depKeys.includes("nestjs") || depKeys.includes("fastify") || depKeys.some(d => d.includes("nestjs"))) {
        stack.type = "backend";
        if (depKeys.includes("express")) stack.frameworks.push("Express");
        if (depKeys.includes("nestjs") || depKeys.some(d => d.includes("@nestjs"))) stack.frameworks.push("NestJS");
      } else {
